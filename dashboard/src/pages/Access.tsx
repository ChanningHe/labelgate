...
import { useAccess } from '../hooks/useAPI';
...
export function Access() {
  ...
  const { data: apiData, error, isLoading } = useAccess(
    Object.keys(queryParams).length > 0 ? queryParams : undefined,
  );

  const useMock = !apiData && !!error && import.meta.env.DEV;
  const resources = useMock ? mockAccessResources : (apiData?.resources ?? []);
  ...
  const desktopDetailPanel = (
    <Box pos="sticky" top={80}>
      <Paper withBorder radius="md" h="100%" style={{ minHeight: 400 }} p="lg">
        <ResourceDetailContent resource={selectedResource} type="access" />
      </Paper>
    </Box>
  );
  ...
  const mobileDrawer = (
    <Drawer
      ...
      position="right"
      size="md"
      title="Access Policy Details"
    >
      <ResourceDetailContent resource={selectedResource} type="access" showHeader={false} />
    </Drawer>
  );
