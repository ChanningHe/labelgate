...
import { useDNS } from '../hooks/useAPI';
import { mockDNSResources } from '../mock/data';
...
export function DNS() {
  ...
  const { data: apiData, error, isLoading } = useDNS(
    Object.keys(queryParams).length > 0 ? queryParams : undefined,
  );

  const useMock = !apiData && !!error && import.meta.env.DEV;
  const resources = useMock ? mockDNSResources : (apiData?.resources ?? []);
  ...
  // Desktop: side panel with Paper wrapper
  const desktopDetailPanel = (
    <Box pos="sticky" top={80}>
      <Paper withBorder radius="md" h="100%" style={{ minHeight: 400 }} p="lg">
        <ResourceDetailContent resource={selectedResource} type="dns" />
      </Paper>
    </Box>
  );

  // Mobile: drawer with content inside
  const mobileDrawer = (
    <Drawer
    ...
    >
      <ResourceDetailContent resource={selectedResource} type="dns" showHeader={false} />
    </Drawer>
  );
  ...
  return (
    <Box maw={1400} mx="auto">
      <Stack gap="lg">
        <Title order={2}>DNS Records</Title>
